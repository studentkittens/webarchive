...
\begin{description}
	...
	\item [\req{Zeitpunkt speichern}{starttime}]
	...
	\item [\req{Normalisierung}{normalize}]
		\begin{description}
			\item [\subreq{Datei umbenennen}{rename}]
				Das HTML-file in ''data.html'' umbenannt. 
			...
			\item [\subreq{Datei verschieben}{move}]
				Die data-Datei wird in den soeben erzeugte \arc\ verschoben.
		\end{description}
	\item [\req{Extraktion der Metadaten}{metaextract}]
		In diesem Vorgang werden die im Datenmodell (Abschnitt \ref{spec:model}) definierten Metadaten 
		aus dem HTML extrahiert und zwischengespeichert. 
		\begin{description}
			\item [\subreq{url}{url}]
		...
		%		Der MIME-Typ der Datei wird mit einer geeigneten Maßnahme ermittelt.
			...
			\item [\subreq{crawlTime}{crawlTime}]
				Es wird der Beginn des Crawlvorgangs (\ref{req:Cr:starttime}) 
				als Timestamp bzw. als Ganzzahl in ms gespeichert.
			\item [\subreq{commitTag}{commitTag}]
				Der commitTag wird aus Domainordnername und crawlTime (im Textformat) zusammen.
		\end{description}
	\item [\req{Erzeugung von XML-Dateien}{xml}]
		...
		geschrieben und jeweils im zugehörigen \arc\ gespeichert. 
...
\begin{itemize}
	...
	\item Die einzelnen HTML-Dateien werden durch \arc-Ordner ersetzt, bzw. in diese verschoben.
	\item Jedes \arc\ enthält die Quell-HTML-Datei, 
...
\begin{description}
	...
	\item [\req{Inhalt}{content}]
        ...
        Die gespeicherten Daten entsprechen den Knoten des Abschnitts \ref{spec:model}.
