...
Die Clients werden anschließend bei Änderungen oder neuen Einträgen benachrichtig und können sich selbstständig über die Schnittstelle einen beliebigen Stand der Daten herunterladen und an Analysetools weitergeben.
...
\subsection{Server} \label{spec:req:jcs:server}
	...
	\subsubsection{Update-Notifier} \label{spec:req:jcs:server:notifier}
		Der Update-Notifier ist ein eigens laufender Thread des Servers, der in einem vorgegebenen Intervall (z.B. stündlich) die Datenbank nach neuen oder aktualisierten Dateien durchsucht.
