...
\subsection{Steuerung} \label{spec:req:crawler:control}
	...
	\subsubsection{Kommandozeileninterface} \label{spec:req:crawler:control:cmd} 
		...
		\begin{itemize}
			...
			\item Es kann ein Datenbank-Recovery erzwungen werden. siehe \ref{spec:req:db:recovery}
		\end{itemize}
...
\subsection{Schnittstelle - Client}\label{spec:req:jcs:client}
	\subsubsection{Registrierung am Server} \label{spec:req:jcs:client:register}
		Alle aktiven Java-Clients werden beim Server gespeichert, um diese über Änderungen informieren zu können.
	...
	\subsubsection{Benachrichtigungen} \label{spec:req:jcs:client:notify}
		Sobald serverseitig Änderungen der archivierten Daten vorliegen, müssen alle Clienten das aktualisierte Datum und ein oder mehrere CommitTags empfangen können.
		Die Clients sollen von Observern beobachtet werden können. Bei diesen Beobachtern kann es sich zum Beispiel um Analysetools handeln, die dann selbst entscheiden können,
		welche Daten sie benötigen und mithilfe der CommitTags entsprechende Datenbankabfragen auslösen. 
		Die Clients erhalten also Updateinformationen vom Server und geben diese an registrierte Observer weiter.
...
\subsection{Server} \label{spec:req:jcs:server}
	...
	\subsubsection{Clienten informieren} \label{spec:req:jcs:server:clientnotify}
		Sobald die Update-Suche fertig ist, werden die gepufferten Informationen an die regisitrierten Clienten geschickt.
		Der weitere Vorgang ist dann unter \ref{spec:req:jcs:client:notify} beschrieben.
